...
public final class TimelineObserver: TransactionObserver, Sendable {
    ...
    private struct State {
        ...
        var continuations: [UUID: AsyncStream<DateInterval>.Continuation] = [:]
        var enabled = true
    }
    ...
    public var enabled: Bool {
        ...
        set {
            let shouldProcess = state.withLock {
                let wasDisabled = !$0.enabled
                $0.enabled = newValue
                return newValue && wasDisabled
            }
            if shouldProcess { processPendingChanges() }
        }
    }
    ...
    private func process(rowIds: [String: Set<Int64>]) async {
        ...
        let query = """
            ...
            """
        do {
            let dateRanges = try await Database.pool.read { db -> [DateInterval] in
                try Row.fetchAll(db, sql: query).compactMap { row -> DateInterval? in
